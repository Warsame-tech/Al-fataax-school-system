...
- **Results visibility toggle** (`Building.resultsVisible`): when off, a **student's own self-view** (`GET /api/results/student/:studentId` as a student) is blocked with a 403 explaining why — verified live against the local DB (403 when off, unaffected for admin, restored on toggle-on). Admin/coordinator/gudoomiye access to the same data is unaffected by the toggle — it only gates the student's own view. Since rank (below) is computed inside this same endpoint, a student whose results are hidden sees no rank either — there's nothing separate to gate.
