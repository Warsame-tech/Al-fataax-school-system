...
|---|---|---|---|
| `Building` | `Masjids` | `id`, `name` | A masjid. No longer carries a `resultsVisible` column — see `SystemSetting` below. |
...
| `SystemSetting` | `system_settings` | `id` (always `1`, no auto-increment), `resultsVisible` (bool, default true) | **Single global row** — the one source of truth for the global Results Visibility switch (see `utils/systemSettings.js`). Not per-masjid, per-stage, or per-role. |
...
Existing migration history (in order): per-masjid `resultsVisible` added → GUDOOMIYE role added → `Student.id` converted from auto-increment integer to string PK (cascading to `Result`/`User`) → multi-stage enrollment backfilled from the old single `classId` (which was then dropped) → FK constraints added for the `through` table → `ON UPDATE CASCADE` added so a Student ID can be renamed → Teacher role/table fully removed → `registrationStatus` added (existing students backfilled to `accepted`) → Forgot Password (`Users.email` + `password_reset_otps`) → per-masjid `resultsVisible` **replaced** by the single global `system_settings` switch (`010-global-results-visibility.js`), the old column dropped.
...
| Role | Scope | Can do |
|---|---|---|
| **admin** | System-wide, unrestricted | Everything: full CRUD on masjids, fans, books, stages, students, coordinators, users; results registration; all reports; the one global Results Visibility switch. |
| **gudoomiye** ("GUDOOMIYE", labeled "Chairman" in some requests) | System-wide, **read + one narrow write** | Views students/results/stats across every masjid (masjid-by-masjid, gender-filtered, all-madrasa). The **only** mutation it can perform anywhere is accepting a pending student (`PATCH /api/gudoomiye/reports/students/:id/accept`) — never edits name/gender/masjid/stage, never deletes. No admin-level editing. Also gets read access to the admin Dashboard summary and several admin list/report endpoints (widened deliberately — see `authorizeRoles` calls across routes). Has a **View Results** page (`/results/view`, same general browse-by-masjid+stage / all-students / search-by-ID format as admin, not restricted to any one masjid or student) — this reaches `/api/results/{by-class,all,search}`, which **are** gated by the global Results Visibility switch below (blocked while OFF, same as coordinator). Deliberately **not** given the Top 3/Top 10 leaderboard or the student self-view endpoint — only the one View Results page was requested. gudoomiye's other own routes (`/api/gudoomiye/reports/*`) don't touch `Result` data at all, so the switch has no effect on those. |
| **coordinator** ("GUDOOMIYE KUXIGEEN") | **Locked to their own assigned masjid**, resolved server-side via `ownBuildingId(req.user)` (`server/src/utils/scoping.js`) | Registers/edits/deletes students **only within their own masjid** (enforced in `studentController` regardless of what a request body/query sends); views results for their own masjid (`resultController.getAll`/`getByClass`/`search` all hard-lock `buildingId`) **when the global Results Visibility switch is ON** — while it's OFF, every `/api/results/*` read route is blocked outright for this role too, same as student (see below). Has two masjid-locked reports (All Students, All Students Results — no masjid picker in the UI, and the backend ignores any `buildingId` sent). Cannot view or select another masjid's data through the UI **or** a direct API call — this is verified server-side, not just hidden client-side. |
| **student** | Their own record only | Views only their own results (`scopeStudentToSelf` middleware overwrites any `:studentId` param with the caller's own linked ID) **when the global Results Visibility switch is ON**. Picks which of their stages to view if enrolled in more than one. No write access anywhere. |

**Global Results Visibility** (`server/src/middleware/enforceResultsVisibility.js`, backed by `SystemSetting`/`utils/systemSettings.js`): one system-wide switch, toggled from a header control visible only to admin (`client/src/components/layout/Topbar.jsx`). Applied as `router.use(enforceResultsVisibility)` on every route in `resultRoutes.js`, so it can't be missed on any individual endpoint. **Admin always bypasses it**; every other role reaching a `/api/results/*` route (student, coordinator, and gudoomiye's View Results page — see roles above) gets a 403 from every read route while it's OFF, with **no** per-masjid/per-stage/per-role exception. This replaced an earlier per-masjid `Building.resultsVisible` toggle (migration `010-global-results-visibility.js` drops that column and seeds the new single settings row) that only ever gated a student's own self-view — the global switch now gates coordinator's results access too, a deliberate behavior change from the earlier design.
...
- **Global Results Visibility switch** (see the "Global Results Visibility" note under Roles & Permissions above): one system-wide ON/OFF, admin always exempt, every other role blocked from every `/api/results/*` read route while OFF. Verified live against the local DB in both directions. Since rank (below) is computed inside these same endpoints, a blocked role sees no rank either — there's nothing separate to gate.
- **Stage-based student ranking** (`resultController.computeStageRankMap`): ranks a Stage's students by `average`, live from whatever `Result` rows currently exist — **never persisted**, since this schema has no "result set/period" concept (see migration history above); recomputing live is what guarantees an old and a new round of marks entered at different times can never mix, as there is only ever one current state to rank. Ranking is **always cross-masjid within a Stage** — `computeStageRankMap` never filters by `buildingId`, so a coordinator's own-masjid-scoped table still shows each student's true Stage-wide rank, not a rank relative to just their own masjid. A student only qualifies if they currently have **at least one `Result` for one of that Stage's books**; an enrolled-but-ungraded student is excluded from ranking entirely (`rank: null`), never ranked at 0. Ties use standard competition ranking (1, 2, 2, 4, ...) via the single shared `assignRanks()` in `gradeCalculator.js` — every view that shows a rank calls through this one function, so none can disagree. Wired into `getByClass`, `getAll`, and `buildStudentStageSheet` (so both `getForStudent` and `search` return it).
...
- **Grading bands** (`server/src/utils/gradeCalculator.js`, display-only color mirror in `client/src/utils/gradeUtils.js`'s `gradeToColor`): 11-tier scale — A+ (≥96), A (≥91), A- (≥86), B+ (≥81), B (≥76), B- (≥71), C+ (≥66), C (≥61), C- (≥56), D+ (≥50, passing), else **D** — D *is* the fail grade (below 50), there is no separate F. This 50-point pass/fail line currently matches `FAILING_MARKS_THRESHOLD` in `gradeUtils.js` (an unrelated rule for coloring an individual subject's marks red, not the overall stage grade) — coincidental, not enforced to stay in sync if either changes again. Total/average/grade are always computed server-side; the client never recalculates them, only mirrors the color mapping (green for the A family, gold for B/C/D+, red for D).
- **Top-N leaderboard ties are never split** (`takeTopWithTies()` in `gradeCalculator.js`, used by `resultController.getLeaderboard`): if the score at the Nth position is shared by students ranked beyond N, all of them are included — a "Top 3" can return more than 3 rows when there's a tie for 3rd place. This is the same `assignRanks()` used everywhere else, so a tie is never displayed inconsistently between the leaderboard, the results tables, and a student's own summary.
...
| Results | `/api/results` | Every route below `authenticate` also passes through `enforceResultsVisibility` (admin exempt, everyone else blocked while the global switch is OFF — see Roles & Permissions above). `GET /by-class` admin/coordinator/gudoomiye (masjid-scoped for coordinator only); `GET /all` admin/coordinator/gudoomiye (masjid-scoped for coordinator only); `GET /leaderboard` admin/coordinator (top-N per Stage, tie-inclusive via `takeTopWithTies` — coordinator scoped to own masjid's students but with their true cross-masjid rank; **not** given to gudoomiye); `GET /search` admin/coordinator/gudoomiye (masjid-scoped for coordinator only); `GET /student/:studentId` admin/student (self-scoped for student, **not** gudoomiye); writes admin-only |
...
| Settings | `/api/settings` | Admin-only: `GET /results-visibility`, `PUT /results-visibility` — the one control surface for the global Results Visibility switch |
...
- `/results/view` — shared by admin and gudoomiye (browse by masjid+stage / all-students / search-by-ID modes, system-wide for both), coordinator (search-by-ID only, own masjid), student (own results, stage picker if multi-stage)
