...
- **Grading bands** (`server/src/utils/gradeCalculator.js`, display-only color mirror in `client/src/utils/gradeUtils.js`'s `gradeToColor`): 10-tier scale in 5-point bands — A+ (≥96), A (≥91), A- (≥86), B+ (≥81), B (≥76), B- (≥71), C+ (≥66), C (≥61), C- (≥56), else **D** — D *is* the fail grade, there is no separate F. This 56-point pass/fail line is intentionally different from `FAILING_MARKS_THRESHOLD` (50) in `gradeUtils.js`, which is an unrelated rule for coloring an individual subject's marks red, not the overall stage grade. Total/average/grade are always computed server-side; the client never recalculates them, only mirrors the color mapping (green for the A family, gold for B/C, red for D).
