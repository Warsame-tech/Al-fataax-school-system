...
- **Grading bands** (`server/src/utils/gradeCalculator.js`, display-only color mirror in `client/src/utils/gradeUtils.js`'s `gradeToColor`): 13-tier scale — A+ (≥94), A (≥90), A- (≥86), B+ (≥82), B (≥78), B- (≥74), C+ (≥70), C (≥66), C- (≥62), D+ (≥58), D (≥54), D- (≥50), else F. The 50-point pass/fail line matches `FAILING_MARKS_THRESHOLD` in `gradeUtils.js`. Total/average/grade are always computed server-side; the client never recalculates them, only mirrors the color mapping (green for the A family, gold for B/C, red for D/F).
