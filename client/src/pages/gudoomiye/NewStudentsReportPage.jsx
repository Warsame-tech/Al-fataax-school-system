...
export default function NewStudentsReportPage() {
  ...
  const fetchReport = useCallback(async () => {
    setLoading(true);
    try {
      const data = await gudoomiyeReportsApi.newStudents({
  ...
  return (
    <div>
      ...
      <div className="mb-6 grid grid-cols-1 gap-4 sm:max-w-3xl sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">From</label>
          <input
            type="date"
            value={from}
        ...
        <div>
          <label className="mb-1 block text-sm font-medium text-gray-700 dark:text-gray-200">To</label>
          <input
            type="date"
            value={to}
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="mb-6 rounded-xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-700 dark:bg-gray-800">
            <h2 className="mb-4 text-lg font-semibold text-brand-red dark:text-red-400">By Masjid</h2>
                ...
                <table className="w-full min-w-max text-left text-sm">
                  <thead>
                    <tr className="border-b border-gray-200 bg-brand-red/5 dark:border-gray-700 dark:bg-brand-red/10">
                      ...
                      <th className="whitespace-nowrap px-4 py-2.5 font-semibold text-brand-red dark:text-red-400">Total</th>
                      <th className="whitespace-nowrap px-4 py-2.5 font-semibold text-brand-red dark:text-red-400">Male</th>
                      <th className="whitespace-nowrap px-4 py-2.5 font-semibold text-brand-red dark:text-red-400">Female</th>
                    </tr>
                  </thead>
                  <tbody>
                    {report.byBuilding.map((b) => (
                      <tr key={b.buildingId} className="border-b border-gray-100 last:border-0 dark:border-gray-700">
                        ...
                        <td className="whitespace-nowrap px-4 py-2.5 text-gray-700 dark:text-gray-100">{b.male}</td>
                        <td className="whitespace-nowrap px-4 py-2.5 text-gray-700 dark:text-gray-100">{b.female}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
